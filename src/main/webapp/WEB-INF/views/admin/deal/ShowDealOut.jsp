...
<c:url var="newURL" value="/quan-tri/listdanhsach"/>
<c:url var="newURL1" value="/quan-tri/listdanhsachxong"/>
...
<html>
	...
	<body>
		...
		<div class="main-content">
				...
				<div class="main-content-inner" >
					...
					<div style="height: 200px;width:1270px;border-color: #F08080;">
	...
	<a>	<img  src="<c:url value='/template/Picture1.png' />" style="height: 150px;width: 150px;position: absolute;left: 110px;top:0px"></img></a>
					<p style="font-size: 30px;color:black;font-weight: 700;position: absolute;left: 400px;top:0px">HOÁ ĐƠN GIÁ TRỊ GIA TĂNG</p>
						...
						<p style="font-size: 20px;color:black;font-weight: 700;position: absolute;left: 1000px;top:60px" id="hvn">Ký hiệu : <font style="color:black">000000${model.makh }</font> </p>
					...
					<p style="font-size: 18px;color:black;position: absolute;left: 20px;top:90px" id="hvn">Địa Chỉ (Address) : &emsp;<font style="color:black;font-size: 17px;font-weight: 700"> Số 7,Hồ Thiền Quang ,Hai Bà Trưng,Hà Nội</font></p>
						...
						<p style="font-size: 18px;color:black;position: absolute;left: 20px;top:170px" id="hvn">STK (Account No) : &emsp;<font style="color:black;font-size: 17px;font-weight: 700">Thanh toán qua ví điện tử</font> </p>
							...
							<div style="height: 200px;width:1270px;border-color: #F08080;border-width: 1px;border-style: solid ;position: relative;left: 5px">
					<p style="font-size: 18px;color:black;position: absolute;left: 20px;top:10px"> Người Mua (Buyer) :  <font style="color:black;font-size: 17px;font-weight: 700">${model.user.fullName }</font></p>
						<p style="font-size: 18px;color:black;position: absolute;left: 20px;top:50px" id="hvn">Địa Chỉ (Adress) :<font style="color:black;font-size: 17px;font-weight: 700"> ${model.user.diachi }</font></p>
					<p style="font-size: 18px;color:black;position: absolute;left: 20px;top:90px" id="hvn">Số Điện Thoại (Tel) : <font style="color:black;font-size: 17px;font-weight: 700"> ${model.user.sdt }</font></p>
						
						<p style="font-size: 18px;color:black;position: absolute;left: 20px;top:130px" id="hvn">Gmail (Gmail) : <font style="color:black;font-size: 17px;font-weight: 700">${model.user.gmail }</font></p>
											...
											<table class="table table-bordered" style=" border: 1px solid pink" >
												...
												<tbody >
													<c:set var="count" value="${0}" /> 
														<c:forEach var="item1" items="${model.showdeal}">
														  <c:set var="count" value="${count +1}" /> 
														<tr>
														    <td style=" border: 1px solid  #F08080" id="cot">${count}</td>  
														    <td style=" border: 1px solid  #F08080"><img src="<c:url value='/resources/images/${item1.news.afterimage}'/>" style="height: 50px;width: 80px"/></td>
					...
					<p style="font-size: 15px;color:black;font-weight: 700;position: relative;left: 800px;top:10px" id="hvn">Công Tiền Hàng (sub total) : ${model.giahoadon }.000VND</p>
					...
					<hr style="  border: 1px solid  #F08080;position: relative;top: 10px">
											<p style="font-size: 15px;color:black;font-weight: 700;position: relative;left: 720px;top:0px" id="hvn">Tổng Cộng Tiền Thanh Toán (Total Payment) : ${model.giahoadon }.000VND</p>
					...
					<hr style="  border: 1px solid  #F08080;position: relative;top: 0px">
						...
						<div style="height: 300px"></div>
								<p style="font-size: 18px;color:black;position: relative;left: 150px;top:-280px;font-weight: 700">Đơn vị mua hàng (Buyer)  &emsp;</p>
											...
											<button id="buton1" onclick="accept(${model.id})" style="height: 50px;left: 700px;border-color: rgb(80, 199, 193);position: relative;width: 220px;top: -90px">Xác nhận đơn hàng</button>
											<button onclick="warningBeforeDelete()" style="height: 50px;left: 700px;border-color: rgb(80, 199, 193);position: relative;width: 220px;top: -90px">Xoá đơn hàng</button>
	...
	document.getElementById("demo").innerText="Ngày "+date+" Tháng "+month+" Năm "+year;
		        ...
		        $.ajax({
		            ...
		            success: function (result) {
		                ...
		                window.location.href = "${newURL}?message=delete_success";
		            		 ...
		            		 window.location.href = "${newURL1}?message=delete_success";
		            	}
		            },
		            error: function (error) {
		            	...
		            	window.location.href = "${newURL}?message=error_system";
		            		...
		            		window.location.href = "${newURL1}?message=error_system";
			...
			function accept(id) {
		        ...
		        $.ajax({
		            ...
		            success: function (result) {
		            	if(${model.tt}==0){
		                window.location.href = "${newURL}?message=accept_success";
		            	}
		            	else{
		            		  window.location.href = "${newURL1}?message=accept_success";
		            	}
		            },
		            error: function (error) {
		            	if(${model.tt}==0){
		            	window.location.href = "${newURL}?message=accept_system";
		            	}
		            	else{
		            		window.location.href = "${newURL1}?message=accept_system";
		            	}
		            }
		        });}
				else{
					...
					swal({
						 type: "error",
						  ...
						  footer: 'Làm ơn check lại kho hàng'
						})
				}
